...
# Function to read a NetCDF file and extract data for a specific point
def read_nc_file(file_path, lon_point, lat_point):
...
def read_nc_file_grid(file_path):
    # Open the NetCDF file
    dataset = nc.Dataset(file_path, 'r')
...
# Replace 'your_folder' with the path to your folder containing .nc files
folder_path = 'hdf5/dataset-insitu-gridded-observations-global-and-regional-bcbbce6d-9ad0-4602-b2e4-d54f6f0e3bea'
...
# Initialize lists to store aggregated time and precipitation data for the point
all_times = []
all_pr_points = []
...
df.to_csv('pr_lon109_lat39.csv', index=False)
...
plt.tight_layout()
plt.savefig('decomposed_time_series.png')
plt.show()
...
plt.savefig('time_averaged_precipitation_heatmap.png')
plt.show()
