...
def corr_join(t_series, n: int, h: int, T: float, k_s: int, k_e: int, k_b: int):
  ...
  while alpha*h <= (len(t_series[0])-n):  # I assume all time series have the same length
    ...
    t_0 = perf_counter_ns()   # Time before shifting the window
    for p in range(m):
    ...
    t_1 = perf_counter_ns()   # Time before SVD
    W_b = custom_svd(W_s, k_b)
    t_2 = perf_counter_ns()   # Time before bucketing filter
    C_1, _ = bucketing_filter(W_b, k_b, epsilon_1)
    C_2 = set()

    t_3 = perf_counter_ns()   # Time before Euclidean distance filter
    ...
    t_4 = perf_counter_ns()   # Time before computing the Pearson correlation
    ...
    t_5 = perf_counter_ns()   # Time after computing the Pearson correlation
