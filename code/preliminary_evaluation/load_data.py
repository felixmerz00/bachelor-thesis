...
def trim_length_df(df_time_series: pd.DataFrame,
  round_by: int = 1000) -> pd.DataFrame:
  """
  Trim the length of each row to have same round length.
  """
  l = (df_time_series.shape[1] // round_by) * round_by
  df_time_series = df_time_series.truncate(after=l-1, axis=1)
...
def load_gdrive_chlorine():
  """
  The chlorine dataset has the shape (2040, 4830).

  Returns: pd.DataFrame
  """
  # Use raw string to suppress unnecessary warning.
  df_chlorine = pd.read_csv("./data/google-drive/chlorine.txt", sep=r'\s+', header=None)
  return trim_length_df(df_chlorine, 100)


if __name__ == '__main__':
  load_gdrive_chlorine()
