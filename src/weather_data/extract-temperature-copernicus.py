...
def read_nc_file(file_path: str, lon_point: float, lat_point: float):
  ...
  print(f"longitudes: {dataset.variables['lon'][:]}")
  print(f"latitudes: {dataset.variables['lat'][:]}")
  print(f"lon: {lon_idx}, lat: {lat_idx}")
...
folder_path = 'data/weather/copernicus-temperature-dataset'
...
for file_path in nc_files:
  print(f"Current file: {file_path[-13:-9]}")
...
# data = {
#     'Time': all_times,
#     'Temperature': all_pr_points
# }
# df = pd.DataFrame(data)
# df.to_csv(f"data/weather/copernicus-temperature-script-files/temp_lon{lon_point}_lat{lat_point}.csv", index=False)
