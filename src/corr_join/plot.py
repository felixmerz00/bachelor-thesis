...
def t_runtime_pr_2(df, ds_name: str, m: int, params):
  """
  Plot a double plot, where both plots use the correlation threshold (T) as
  the variable paramter. T is plotted against the total runtime in plot one and
  against the pruning rate in plot 2.
  ...
  # Get unique algorithms from the data
  algorithms = result_df['algorithm'].unique()
  algorithms = algorithms[algorithms != "corr_join"]
  # Generate a color map for the algorithms
  color_map = plt.get_cmap('tab10')(np.linspace(0, 1, max(10, len(algorithms))))
  ...
  plt.tight_layout()
  plt.subplots_adjust(bottom=0.25)  # Make room for the description
  plt.savefig(f"./src/corr_join/plots/t_runtime_pr_{ds_name}_2.png")
  plt.close()
...
def n_runtime(df, ds_name: str, m: int, params):
  """
  Plot with the variable paramter n, which is plotted against the total 
  runtime.
  ...
  plt.tight_layout()
  plt.subplots_adjust(bottom=0.25)  # Make room for the description
  plt.savefig(f"./src/corr_join/plots/n_runtime_{ds_name}.png")
  plt.close()
...
def main():
  ...
  # n_runtime(df, "synthetic", m, util.get_params("n_runtime_pr_plot"))
  # h_runtime(df, "synthetic", m, util.get_params("h_runtime_plot"))

  t_runtime_pr_2(df, "gas", m, util.get_params("t_runtime_pr_plot"))
